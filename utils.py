...
def compute_obj(X1,X2,R,S,W1,W2,H1,H2,D1,A1,D2,A2,alpha,gamma,gamma2,theta1,theta2):
	...
	obj += gamma2 * (torch.trace(W1.T @ R @ W2))
...
def compute_obj_3_R(X1, X2, X3, R12,R13,R23,S, W1, W2, W3, H1, H2, H3, D1, A1, D2, A2, D3, A3, alpha, gamma, beta1,beta2,beta3,theta1,theta2, theta3):
	...
	obj += beta1*torch.trace(W1.T @ R12 @ W2)+beta2*torch.trace(W1.T @ R13 @ W3)+beta3*torch.trace(W2.T @ R23 @ W3)
	return obj



from sklearn import metrics
def cluster_indicators (pred, labels=None, embedding=None ,model_name='cluster', verbose=1):

    measure_dict = dict()
    if labels is not None:
        measure_dict['nmi'] = metrics.normalized_mutual_info_score(labels, pred)
        measure_dict['ari'] = metrics.adjusted_rand_score(labels, pred)
        measure_dict['asw'] = metrics.silhouette_score(embedding, labels)
...
def CH_S(labels=None, embedding=None ,model_name='cluster', verbose=1):

    measure_dict = dict()
    if labels is not None:
        measure_dict['ch'] = metrics.calinski_harabasz_score(embedding, labels)
        measure_dict['asw'] = metrics.silhouette_score(embedding, labels)
...
import numpy as np
def RAGI(data, ind1, ind2, clust):
	# if isspmatrix(data):
	# 	data = np.array(data.todense())
	# else:
	# 	data = np.array(data)
	data = np.array(data.todense())
	num_clu = len(np.unique(clust))

	df_matrix_marker = data[ind1, :]
	df_matrix_housekeeping = data[ind2, :]

	df_marker = np.concatenate((clust.reshape(-1, 1), df_matrix_marker.T), axis=1)
	pop = np.bincount(df_marker[:, 0].astype(int))
	ave_marker = np.zeros((num_clu, len(ind1)))
	marker_gini = np.zeros(len(ind1))

	for i in range(1, len(ind1) + 1):
		ave_marker[:, i - 1] = (np.bincount(df_marker[:, 0].astype(int), weights=df_marker[:, i]) / np.bincount(df_marker[:, 0].astype(int)))[1:]
		marker_gini[i - 1],a,l= gini(pop[1:], ave_marker[:, i - 1])

	df_housekeeping = np.concatenate((clust.reshape(-1, 1), df_matrix_housekeeping.T), axis=1)
	ave_housekeeping = np.zeros((num_clu, len(ind2)))
	housekeeping_gini = np.zeros(len(ind2))

	for j in range(1, len(ind2) + 1):
		ave_housekeeping[:, j - 1] = (np.bincount(df_housekeeping[:, 0].astype(int),weights=df_housekeeping[:, j]) / np.bincount(
			df_housekeeping[:, 0].astype(int)))[1:]
		housekeeping_gini[j - 1],a,l = gini(pop[1:], ave_housekeeping[:, j - 1])

	ave_marker_gini = np.mean(marker_gini)
	ave_housekeeping_gini = np.mean(housekeeping_gini)
	diffgini = ave_marker_gini - ave_housekeeping_gini

	return ave_marker_gini, ave_housekeeping_gini, diffgini
...
def gini(pop, val, makeplot=False):
	assert len(pop) == len(val), "gini expects two equally long vectors."

	pop = np.insert(pop, 0, 0)
	val = np.insert(val, 0, 0)

	isok = ~np.isnan(np.vstack((pop, val)).T).any(axis=1)
	if np.sum(isok) < 2:
		print("Not enough data")
		return np.nan, np.nan * np.ones(4)

	pop = pop[isok]
	val = val[isok]

	assert (pop >= 0).all() and (val >= 0).all(), "gini expects nonnegative vectors."
	...
	relpop = pop_cumsum / pop_cumsum[-1]
	relz = z_cumsum / z_cumsum[-1]

	g = 1 - np.sum((relz[:-1] + relz[1:]) * np.diff(relpop))

	l = np.vstack((relpop, relz)).T
	a = np.vstack((pop_cumsum, z_cumsum)).T

	if makeplot:
		plt.figure()
		plt.fill_between(relpop, relz, facecolor='lightblue')
		plt.plot([0, 1], [0, 1], '--k')
		plt.axis('tight')
		plt.axis('square')
		...
		plt.grid(True)
		plt.title('Gini coefficient = {}'.format(g))
...
def featureRankingPlot(W, features, Topnmarkers, Topgenes=None, Topnnames=None, index=None, top_per=None,folder='figures/brain/'):
	if index is None:
	...
	K = W.shape[1]
	W = W / np.tile(np.sum(W, axis=1, keepdims=True), (1, K))
	# W = W / np.tile(np.sum(W, axis=1)[:, np.newaxis], (1, K))
	W_order = np.copy(W)
	Indexs = []
	for i in range(K):
		W_order[:, i], indices = np.sort(W[:, i]), np.argsort(W[:, i])
		Indexs.append(indices)

	fig, axs = plt.subplots(1, len(index), figsize=(8,8))
	markpeak_in_factor = []
	for i in range(len(index)):
		# featurei = features[Indexs[index[i] - 1]]
		featurei = features[Indexs[index[i]]]
		if not isinstance(Topnmarkers, list):
			ai, idi, idj = np.intersect1d(featurei, Topnmarkers, return_indices=True)
		else:
			ai, idi, idj = np.intersect1d(featurei, Topnmarkers[i], return_indices=True)

		if top_per:
			idi_index = np.where(idi >= top_per * W.shape[0])
			idi, idj, ai = idi[idi_index], idj[idi_index], ai[idi_index]

		if Topnnames and not isinstance(Topnnames, list):
			ai = Topnnames[idj]
		elif Topnnames:
			ai = Topnnames[index[i]][idj]
			# ai = Topnnames[index[i] - 1][idj]

		if len(index)==1:
			ax=axs
		else :
			ax = axs[i]
		# wi = W_order[:, index[i] - 1]
		wi = W_order[:, index[i]]
		ax.plot(np.arange(len(wi)), wi, color=[0.8, 0.8, 0.8], linewidth=2)
		ax.set_xlabel('Ranking', fontsize=20)
		if not Topgenes:
			ax.set_ylabel('Gene score', fontsize=20)
		else:
			ax.set_ylabel('Locus score', fontsize=20)

		ax.set_xticks([1000, 3000, 5000])
		ax.set_xticklabels(['1k', '3k', '5k'])
		ax.set_yticklabels([])
		ax.set_title('Factor ' + str(index[i]+1), fontsize=20)

		markpeak_in_factor.append(ai.tolist())
		for j in range(len(idi)):
			ax.plot(idi[j], wi[idi[j]], 'o', markersize=4, color='k')
			ax.text(idi[j] - 0.02 * idi[j], wi[idi[j]], ai[j], fontsize=16, horizontalalignment='right')

	plt.tight_layout()
	# plt.show()
	plt.savefig(folder+'featureRanking.pdf', format='pdf', dpi=300, bbox_inches='tight')

	return markpeak_in_factor
